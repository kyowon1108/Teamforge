...
const STRENGTH_AREAS = ["백엔드", "프론트엔드", "데이터베이스", "인프라/DevOps", "AI/ML", "디자인"];
...
export default function Section2TechStack({ answers, updateAnswers }: Props) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        {Object.entries(TECH_CATEGORIES).map(([category, techs]) => (
          <div key={category} className="space-y-2">
            <p className="text-[12px] font-medium text-[var(--tf-fg-muted)] uppercase tracking-wide">
              {category}
            </p>
      ...
      {/* Q4: Skill ratings for selected techs */}
      {techStackList.length > 0 && (
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {techStackList.map((tech) => (
              <div key={tech} className="flex items-center gap-3">
                <span className="text-[13px] text-[var(--tf-fg-default)] w-28 shrink-0 truncate">
                  {tech}
                </span>
                <div className="flex gap-1 flex-1">
                  {SKILL_LEVELS.map((level) => (
                    <button
                      ...
                      title={level.label}
                      className={`
                        flex-1 h-8 rounded-r1 text-[11px] transition-all
                        ${(skillRatings[tech] ?? 0) >= level.value
                          ? "bg-[var(--tf-bg-brand-solid)] text-[var(--tf-fg-inverse)]"
                          : "bg-[var(--tf-bg-layer-alt)] text-[var(--tf-fg-subtle)]"
                        }
                      `}
                    >
                      {level.value}
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-[var(--tf-fg-subtle)]">
            {SKILL_LEVELS.map((l) => (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          {STRENGTH_AREAS.map((area) => (
            <button
              ...
              className={`
                h-10 rounded-r2 text-[13px] font-medium transition-all
                ${topStrengths.includes(area)
                  ? "bg-[var(--tf-bg-brand-solid)] text-[var(--tf-fg-inverse)]"
                  : "border border-[var(--tf-stroke-neutral)] text-[var(--tf-fg-muted)] hover:border-[var(--tf-stroke-brand)]"
                }
                ${topStrengths.length >= 2 && !topStrengths.includes(area) ? "opacity-50" : ""}
              `}
            >
              {area}
            </button>
